...
test('Verify form validation message', async ({ page }) => {
    ...
    expect.soft(await getErrorMessage(page, 'Full Name')).toEqual("Please input your full name!");
    expect.soft(await getErrorMessage(page, 'Email')).toEqual("Please input your email!");
    expect.soft(await getErrorMessage(page, 'Phone Number')).toEqual("Please input your phone number!");
    expect.soft(await getErrorMessage(page, 'Date of Birth')).toEqual("Please select your date of birth!You must be at least 18 years old!");
    expect.soft(await getErrorMessage(page, 'Address')).toEqual("Please input your address!");
    //console.log(test.info().errors)
    expect(test.info().errors).toHaveLength(0);
});


async function getErrorMessage(page: Page, label: string) {
...
test('Verify submit form sussessful', async ({ page }) => {
